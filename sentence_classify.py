# -*- coding: utf-8 -*-
"""chatbot_20180830_2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1BS0nZly25ONwApe8cpezwPMQ9TugeaP2
...
# 似た単語で置き換えてCSVデータ追加
results = wtv.wv.most_similar(positive=['青木さやか','ポジティブ'],negative=['ネガティブ'])
print(results)
...
class WordDividor:
    ...
    def extract_words(self, text):
        ...
        while node:
            ...
            node = node.next
        increase_chat_data(words)
        #print(words)
        return words

import copy

def increase_chat_data(words):
    new_data = []
    ...
    for i,word in enumerate(words):
        try:
            ...
            for result in results:
                cmp = compare_similar_word(pre_data[i],result[0])
...
def compare_similar_word(pre_word,new_word):
    INDEX_PART = 0 # 品詞
...
#ラベル(分類を作る)
le = LabelEncoder()
classify_label=le.fit_transform(label) # fit_transformでdataframe型になっているラベルをfitできる形に変形
...
!7z a chatbot.7z appear.pkl le.pkl model.pkl
...
#1202!pip install -q pydot
!apt-get -qq install -y graphviz && pip install -q pydot
import pydot

from keras.utils.vis_utils import plot_model
plot_model(model_keras, to_file='model.png')
